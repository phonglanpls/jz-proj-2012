...
<?php 
	...
	$query = "SELECT * FROM ".TBL_BLOCKIP." WHERE time_fail != '0000-00-00 00:00:00' " ;
	$cond = '';
	if($username){
	...
	$query .= " ORDER BY id_block DESC LIMIT $offset,$rec_per_page";
	
	$record = $this->db->query($query)->result();
	
	$pagination = create_pagination( 
					$uri = "admin/juzon/report/log_login/?username=$username", 
					$total_rows = $total , 
					$limit= $rec_per_page,
					$uri_segment = 0,
					TRUE, TRUE 
...
<section class="item">
	...
	<table border="0" cellpadding=0 class="table-list clear-both" width="100%">
		...
		<tbody>
			...
			?> 	
				<tr id="row_<?php echo $item->id_block;?>"> 	 
					<td><?php echo form_checkbox('action_to[]', $item->id_block);?></td>
					<td><?php echo $item->username;?></td>
					<td><?php echo $item->ip;?></td>
					<td><?php echo $item->time_fail;?></td> 	 	
...
<script type="text/javascript">
	function callFuncDeleteItem(){
		...
		$.post(BASE_URI+'admin/juzon/report/callFuncDeleteLoginLogItem',{id_str:str},function(res){
