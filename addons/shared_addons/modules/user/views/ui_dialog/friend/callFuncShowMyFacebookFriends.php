...
<?php 
	$maxlength = 200;
	echo form_open(
						$action = site_url("mod_io/friend_async/submit_invite_facebook_friend"), 
						$attributes = "method='post' id='submit_invite_facebook_friend' name='submit_invite_facebook_friend' ", 
						$hidden = array()
				);
?>
...
<div id="wrap-dialog-box">	
	<div class="input" id="questionArea">
		...
		<div class="clear"></div>
		<label>&nbsp;</label> <textarea class="disablecopypaste" maxlength="<?php echo $maxlength;?>" cols="15" rows="5" style="width:392px;height:45px;" id="message" name="message"><?php echo $GLOBALS['global']['FACEBOOK']['FirstStatusMessage'];?></textarea>
	...
	<div class="input" id="fbFriends">
		<?php $i=0; foreach($this->facebookconnect_io_m->getFriendsList(getAccountUserId()) as $k => $v):?>
			<?php
				foreach($v as $item) : 
					$item = (array)$item;
			?>
				<div class="wrap-left" style="width:23%;">
					<div class="user-profile-avatar">
						<?php echo "<img src=".$this->facebookconnect_io_m->getFacebookUserProfileImage($item['id'])." />";?>
					</div>
				 
					<div class="user-profile-owner">
						<?php echo $item['name']?>
						<div class="clear"></div>
						<input type="checkbox" name="usercheck[]" value="<?php echo $item['id'];?>"/>
					</div>
				</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#submit_invite_facebook_friend').ajaxForm(options); 
	});
