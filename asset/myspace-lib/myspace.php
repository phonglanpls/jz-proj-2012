...
*/
  ...
  public function getAlbum($userId, $albumId){
	...
	$params = array( 'imageUrl' => null);
  	$responseJSON = $this->call($REST, $params);
  ...
  */
  ...
  public function getAuthorizeURL($token) {
	...
	return 	(
		...
		'?oauth_token=' .
		$token .
		'&myspaceid.permissions=ViewFullProfileInfo|UpdateMoodStatus|AddPhotosAlbums|ShowUpdatesFromFriends|AccessToPrivateVideosPhotos&oauth_callback=http://' .
		$_SERVER['HTTP_HOST'] .
...
function generate_token($key,$secret,$callback)
    {
    ...
    $request = new OAuthRequest('GET','http://api.myspace.com/request_token',NULL);
   

    $request = $request->from_consumer_and_token($consumer, $token ,'GET','http://api.myspace.com/request_token', NULL);
   

    $signature = new OAuthSignatureMethod_HMAC_SHA1($request, $consumer, $token);
   
   
    $request->sign_request($signature, $consumer, $token);
   
   
    $url_for_token = $request->to_url();
    
    //    return $url_for_token;
   $res = $this->http($url_for_token);
   //print $url_for_token;
  ...
  // print '<pre>';print_r($tok);exit;
    return $tok;
    }

function generate_access_token($key,$secret)
    {
    ...
    $request = new OAuthRequest('GET','http://api.myspace.com/access_token',NULL);
   

    $request = $request->from_consumer_and_token($consumer, $token ,'GET','http://api.myspace.com/access_token', NULL);
