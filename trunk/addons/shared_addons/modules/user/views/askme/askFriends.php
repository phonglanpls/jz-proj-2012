...
<?php 
	...
	$total = count( $this->friend_m->getAllFriends(getAccountUserId()) );
	...
	$myFriends = $this->friend_m->getAllFriends(getAccountUserId(),$offset,$rec_per_page);
	
	$pagination = create_pagination( 
					$uri = "user/qa_func/callFuncShowAksFriends/?is_async=1", 
					$total_rows = $total , 
					$limit= $rec_per_page,
					$uri_segment = 0,
					TRUE, TRUE 
		...
		<?php $i=1; foreach($myFriends as $item):?>
			<div class="friend-item-s">
				...
				<div class="user-profile-button">
					<a href="javascript:void(0);" onclick="callFuncShowDialogSubmitQuestion(<?php echo $item->id_user;?>);">
						<?php echo language_translate('askfriend_askme');?>
					</a>
				</div>
				
				<div class="user-profile-button">
					<a href="javascript:void(0);" onclick="callFuncShowDialogViewLog(<?php echo $item->id_user;?>);">
						<?php echo language_translate('askfriend_viewlog');?>
					</a>
				</div>
			</div>
