...
class Videos extends Public_Controller
{
	...
	public function __construct()
	{
		...
		$this->template
			...
			->append_metadata( js('swf.js', 'videos') )	
			->append_metadata( js('hentai.js', 'videos') );
		...
		if(isLogin()){
			$currenturl = fullURL();
			if(false !== strpos($currenturl,'/videos/category/')){
				$redr = str_replace( '/videos/category/','/user/videos/',$currenturl);
				redirect($redr);
				die; 
			}
